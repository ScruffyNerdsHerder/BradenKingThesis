...
@threads for i in eachindex(C_sweep)
    C = C_sweep[i]
    solver_CSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaCSweepExpDecrease(C, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
    error_threshold = [0.0, 0.0, 0.0]
    print_iter=false
    # Run the C sweep on the Sine 2D benchmark
        Theta_1, res_history_1, _, _, _, _, _ = train_RBFN(
        X_Sine, y_Sine, A_Sine, D_Sine,
        N_max=N_max,
        solver=solver_CSweep,
    ...
    # Run the C sweep on the SineE benchmark
        Theta_1, res_history_1, _, _, _, _, _ = train_RBFN(
...
X_2D = X_Sine
@save "2D_BenchmarkExpCSweep.jld2" X_2D C_sweep resData_2_1 finalTheta_2_1 resData_2_2 finalTheta_2_2 resData_2_3 finalTheta_2_3 resData_2_4 finalTheta_2_4 resData_2_5 finalTheta_2_5 resData_2_6 finalTheta_2_6
end