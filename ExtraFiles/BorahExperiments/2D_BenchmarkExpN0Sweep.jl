...
resData_2_1 = zeros(length(N0_sweep),N_max+1)
finalTheta_2_1 = zeros(length(N0_sweep),N_max,5)
resData_2_2 = zeros(length(N0_sweep),N_max+1)
finalTheta_2_2 = zeros(length(N0_sweep),N_max,5)
resData_2_3 = zeros(length(N0_sweep),N_max+1)
finalTheta_2_3 = zeros(length(N0_sweep),N_max,5)
resData_2_4 = zeros(length(N0_sweep),N_max+1)
finalTheta_2_4 = zeros(length(N0_sweep),N_max,5)
resData_2_5 = zeros(length(N0_sweep),N_max+1)
finalTheta_2_5 = zeros(length(N0_sweep),N_max,5)
resData_2_6 = zeros(length(N0_sweep),N_max+1)
finalTheta_2_6 = zeros(length(N0_sweep),N_max,5)

@threads for i in eachindex(N0_sweep)
    N0 = N0_sweep[i]
    solver_N0Sweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaN0SweepExpDecrease(N0, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
    ...
    # Run the N0 sweep on the Peaks benchmark
        Theta_1, res_history_1, _, _, _, _, _ = train_RBFN(
    ...
    # Run the N0 sweep on the Spiral benchmark
        Theta_1, res_history_1, _, _, _, _, _ = train_RBFN(
...
X_2D = X_Sine
@save "2D_BenchmarkExpN0Sweep.jld2" X_2D N0_sweep resData_2_1 finalTheta_2_1 resData_2_2 finalTheta_2_2 resData_2_3 finalTheta_2_3 resData_2_4 finalTheta_2_4 resData_2_5 finalTheta_2_5 resData_2_6 finalTheta_2_6
end