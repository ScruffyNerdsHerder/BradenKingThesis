...
## Run the LinearIncrease omega sweep on each 1D benchmark and save the results
OmegaFinal_sweep = [0:0.5:20...]
N_max = 250
monotonicity = Nonstrict()

resData_1 = zeros(length(OmegaFinal_sweep),N_max+1)
finalTheta_1 = zeros(length(OmegaFinal_sweep),N_max,4)
resData_2 = zeros(length(OmegaFinal_sweep),N_max+1)
finalTheta_2 = zeros(length(OmegaFinal_sweep),N_max,4)
resData_3 = zeros(length(OmegaFinal_sweep),N_max+1)
finalTheta_3 = zeros(length(OmegaFinal_sweep),N_max,4)
resData_4 = zeros(length(OmegaFinal_sweep),N_max+1)
finalTheta_4 = zeros(length(OmegaFinal_sweep),N_max,4)
resData_5 = zeros(length(OmegaFinal_sweep),N_max+1)
finalTheta_5 = zeros(length(OmegaFinal_sweep),N_max,4)

@threads for i in eachindex(OmegaFinal_sweep)
    # Run the omega sweep on the AsymmetricRBF benchmark
        OmegaFinal = OmegaFinal_sweep[i]
        solver_OmegaSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaFinalSweepLinearIncrease(OmegaFinal, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
...
@save "1D_BenchmarkLinearIncreaseSweep.jld2" X_1D OmegaFinal_sweep resData_1 finalTheta_1 resData_2 finalTheta_2 resData_3 finalTheta_3 resData_4 finalTheta_4 resData_5 finalTheta_5
