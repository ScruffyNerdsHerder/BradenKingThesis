...
Init_sweep = [0:0.05:10...]
N_max = 250
monotonicity = Nonstrict()

resData_1 = zeros(length(Init_sweep),N_max+1)
finalTheta_1 = zeros(length(Init_sweep),N_max,4)
resData_2 = zeros(length(Init_sweep),N_max+1)
finalTheta_2 = zeros(length(Init_sweep),N_max,4)
resData_3 = zeros(length(Init_sweep),N_max+1)
finalTheta_3 = zeros(length(Init_sweep),N_max,4)
resData_4 = zeros(length(Init_sweep),N_max+1)
finalTheta_4 = zeros(length(Init_sweep),N_max,4)
resData_5 = zeros(length(Init_sweep),N_max+1)
finalTheta_5 = zeros(length(Init_sweep),N_max,4)

@threads for i in eachindex(Init_sweep)
    # Run the omega sweep on the AsymmetricRBF benchmark
        Init = Init_sweep[i]
        solver_OmegaSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaInitialSweepLinearDecrease(Init, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
...
@save "1D_BenchmarkLinearInitSweep.jld2" X_1D Init_sweep resData_1 finalTheta_1 resData_2 finalTheta_2 resData_3 finalTheta_3 resData_4 finalTheta_4 resData_5 finalTheta_5
