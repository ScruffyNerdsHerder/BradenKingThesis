...
resData_2_1 = zeros(length(Init_sweep),N_max+1)
finalTheta_2_1 = zeros(length(Init_sweep),N_max,5)
resData_2_2 = zeros(length(Init_sweep),N_max+1)
finalTheta_2_2 = zeros(length(Init_sweep),N_max,5)
resData_2_3 = zeros(length(Init_sweep),N_max+1)
finalTheta_2_3 = zeros(length(Init_sweep),N_max,5)
resData_2_4 = zeros(length(Init_sweep),N_max+1)
finalTheta_2_4 = zeros(length(Init_sweep),N_max,5)
resData_2_5 = zeros(length(Init_sweep),N_max+1)
finalTheta_2_5 = zeros(length(Init_sweep),N_max,5)
resData_2_6 = zeros(length(Init_sweep),N_max+1)
finalTheta_2_6 = zeros(length(Init_sweep),N_max,5)

@threads for i in eachindex(Init_sweep)
    Init = Init_sweep[i]
    solver_InitSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaInitialSweepLinearDecrease(Init, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
    error_threshold = [0.0, 0.0, 0.0]
    print_iter=false
    # Run the Init sweep on the Sine 2D benchmark
        Theta_1, res_history_1, _, _, _, _, _ = train_RBFN(
        X_Sine, y_Sine, A_Sine, D_Sine,
        N_max=N_max,
        solver=solver_InitSweep,
...
X_2D = X_Sine
@save "2D_BenchmarkLinearDecreaseInitSweep.jld2" X_2D Init_sweep resData_2_1 finalTheta_2_1 resData_2_2 finalTheta_2_2 resData_2_3 finalTheta_2_3 resData_2_4 finalTheta_2_4 resData_2_5 finalTheta_2_5 resData_2_6 finalTheta_2_6
end