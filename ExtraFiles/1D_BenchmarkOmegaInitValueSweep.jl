...
resData_1 = zeros(length(Omega_sweep),N_max+1)
finalTheta_1 = zeros(length(Omega_sweep),N_max,4)
...
resData_2 = zeros(length(Omega_sweep),N_max+1)
finalTheta_2 = zeros(length(Omega_sweep),N_max,4)
@threads for i in eachindex(Omega_sweep)
    omega_init = Omega_sweep[i]
    solver_OmegaSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaInitialSweepLinearDecrease(omega_init, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
    error_threshold = [0.0, 0.0, 0.0]
...
resData_3 = zeros(length(Omega_sweep),N_max+1)
finalTheta_3 = zeros(length(Omega_sweep),N_max,4)
@threads for i in eachindex(Omega_sweep)
    omega_init = Omega_sweep[i]
    solver_OmegaSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaInitialSweepLinearDecrease(omega_init, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
    error_threshold = [0.0, 0.0, 0.0]
...
resData_4 = zeros(length(Omega_sweep),N_max+1)
finalTheta_4 = zeros(length(Omega_sweep),N_max,4)
@threads for i in eachindex(Omega_sweep)
    omega_init = Omega_sweep[i]
    solver_OmegaSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaInitialSweepLinearDecrease(omega_init, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
    error_threshold = [0.0, 0.0, 0.0]
...
resData_5 = zeros(length(Omega_sweep),N_max+1)
finalTheta_5 = zeros(length(Omega_sweep),N_max,4)
@threads for i in eachindex(Omega_sweep)
    omega_init = Omega_sweep[i]
    solver_OmegaSweep(theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction}) = lsq_TV_solver_OmegaInitialSweepLinearDecrease(omega_init, theta0, X, res, A, D, N, T_phi::Type{<:BasisFunction})
    error_threshold = [0.0, 0.0, 0.0]
...
@save "OmegaInitialSweep_1D.jld2" X_1D Omega_sweep resData_1 finalTheta_1 resData_2 finalTheta_2 resData_3 finalTheta_3 resData_4 finalTheta_4 resData_5 finalTheta_5
